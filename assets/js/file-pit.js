...
window._lastSelectedFolderName = null; // State proteksi klik berulang
window.hasUnsavedColorChanges = false; // State track apply color

// Ekstensi/Hook ke Framework Folder Global (file.js)
window.getFolderBadgeHTML = function(name, rootName) {
    if (rootName === 'Pit Data') {
        const safeId = name.replace(/\s+/g, '_');
        // Tidak mengandalkan localStorage lagi. Fetch dari DB secara asinkron.
        if (typeof RizpecDB !== 'undefined') {
            RizpecDB.get(`rizpec_entity_${safeId}_meta`).then(meta => {
                if (meta && meta.buildMethod) {
                    window.updateFolderBadge(name, meta.buildMethod);
                }
            }).catch(() => {});
        }
        return ''; // Return kosong di awal, badge diupdate asinkron
    }
    return '';
};
...
window.savePitMetaToDB = async function(pitId) {
    ...
    try {
        if (typeof RizpecDB !== 'undefined') await RizpecDB.set(`rizpec_entity_${safeId}_meta`, meta);
...
window.updateBuildGeometryButtonState = function() {
    ...
    // Disable tombol bin jika user terlanjur Build Geometry
    ...
    const isReconWasteFilled = isFilled(state.cols['col-recon-waste']);
    const isReconResFilled = isFilled(state.cols['col-recon-resource']);
    const isRefValid = isReconWasteFilled && isReconResFilled;
...
window.saveCurrentUIState = async function() {
...
async function restoreUIState(pitId) {
    ...
    try {
        if (typeof RizpecDB !== 'undefined') {
            const meta = await RizpecDB.get(`rizpec_entity_${safeId}_meta`);
            if (meta) {
                state.buildMethod = meta.buildMethod || 'NON_CEN';
                
                // PENTING: Sisipkan kembali ke local storage secara eksklusif 
                // hanya sebagai "SYNC FLAG" agar Web Worker di geometry.js tidak membatalkan render
                ...
                if (meta.mrHeaders) state.mrHeaders = meta.mrHeaders;
    ...
    if (!state.generatedCsv) {
        try {
            const savedCsv = await RizpecDB.get(`rizpec_entity_${safeId}`);
    ...
    if (newEntityNameEl) {
        // [FIXED] Penambahan tanda kurung '(0 Block' agar tidak match dengan '10 Block', '20 Block', dll.
    ...
    if (typeof window.renderGeometryPreview === 'function') {
        window.renderGeometryPreview(state.generatedCsv, state.summaryObj, state.buildMethod);
    }
}
...
window.onFolderSelected = async function(name, type, rootName) {
    // 1. [Fix Kedip] Cegah kalkulasi & render ulang jika user mengklik folder yang sama secara beruntun
    if (window._lastSelectedFolderName === name) return;
    
    // --- BATALKAN PERUBAHAN WARNA SAAT PINDAH FOLDER ---
    ...
    if (window.activePitId && window.activePitId !== name) {
        await window.saveCurrentUIState();
    ...
    // 2. UI Toggling yang lebih Clean untuk membedakan mode Root dan Subfolder
    const container = document.getElementById('geometry-pit-manager');

    if (container) {
        if (rootName === 'Pit Data' && type === 'Root Folder') {
            // Mode ROOT FOLDER: Tampilkan Manager Saja
            ...
            // Mode SUBFOLDER atau Selain 'Pit Data': Sembunyikan Manager
...
window.onFolderRenamed = async function(oldName, newName, rootName) {
    if (rootName === 'Pit Data') {
        // Handle Sinkronisasi State Proteksi Klik
        ...
        // Pindahkan juga flag dari localStorage agar Web Worker sinkron
        ...
        if (window.loadedPits && (window.loadedPits.has(oldName) || window.loadedPits.has(oldNormalized))) {
            ...
            if (typeof window.unloadPitGeometry === 'function') {
                window.unloadPitGeometry(oldName);
                if (oldName !== oldNormalized) window.unloadPitGeometry(oldNormalized);
            }
        }
...
window.onFolderDeleted = async function(name, rootName) {
    if (rootName === 'Pit Data') {
        // Handle Sinkronisasi State Proteksi Klik
        ...
        // Hapus flag penanda Web Worker
        localStorage.removeItem(`rizpec_build_type_${safeName}`);

        try {
            if (typeof RizpecDB !== 'undefined') {
                await RizpecDB.remove(`rizpec_entity_${safeName}`);
                await RizpecDB.remove(`rizpec_entity_${safeName}_meta`);
            }
        } catch(e) {}
        ...
        if (typeof window.unloadPitGeometry === 'function') {
            window.unloadPitGeometry(name);
            if (name !== normalizedPitName) window.unloadPitGeometry(normalizedPitName);
        }
...
function inheritHistoryIfEmpty(state) {
    // Apabila dropdown wajib kosong (artinya Pit baru dibuat), baru panggil history dari Pit sebelumnya.
    const isColEmpty = !state.cols['col-blockname'];
...
async function recalculateHeaders() {
    const state = window.pitStates[window.activePitId];
    if (!state) return;
    ...
    if (state.mrFile && state.mrFile.size !== undefined) {
        promises.push(extractHeaders(state.mrFile).then(data => { 
            state.mrHeaders = data.headers;
            inheritHistoryIfEmpty(state);
            window.populateColumnDropdowns(data.headers, 'mr', data.firstRow); 
            
            // Render ulang kolom input Substrings (Block, Strip, Bench) ke UI agar sinkron dengan histori memori
            const subIds = ['delim-block', 'delim-strip', 'delim-bench'];
            subIds.forEach(id => {
                const el = document.getElementById(id);
                if (el) el.value = state.substrings[id] || '';
    ...
    if (state.refFile && state.refFile.size !== undefined) {
        promises.push(extractHeaders(state.refFile).then(data => { 
...
if (clearMiningBtn) {
    clearMiningBtn.addEventListener('click', async () => {
        ...
        await updateFileStats(null, 'mr'); 
        
        if (typeof RizpecDB !== 'undefined') {
            await RizpecDB.remove(`rizpec_entity_${safePitId}`).catch(()=>{});
            await window.savePitMetaToDB(window.activePitId);
        }
        ...
        if (typeof window.unloadPitGeometry === 'function') {
            window.unloadPitGeometry(window.activePitId);
            if (window.activePitId !== normalizedPitName) window.unloadPitGeometry(normalizedPitName);
        }
...
if (clearReformatBtn) {
    clearReformatBtn.addEventListener('click', async () => {
        ...
        // Reset dropdown Waste dan Resource seketika
        ...
        await updateFileStats(null, 'ref'); 
        await window.savePitMetaToDB(window.activePitId);
...
if (btnBuildGeometry) {
    btnBuildGeometry.addEventListener('click', async () => {
        if (typeof window.saveCurrentUIState === 'function') await window.saveCurrentUIState();
        ...
        try {
            ...
            // Ekstrak string data sebelum dilempar ke Worker
            let mrText = "";
            let refText = "";
            if (action === 'BUILD') {
            ...
            // Web Worker Code (Mengisolasi proses sinkron yang memakan waktu)
            const workerCode = `
                    ...
                    try {
                        ...
                        if (action === 'APPLY_PRORATA' || action === 'RESET') {
                            ...
                            if (action === 'APPLY_PRORATA') {
                                ...
                                const mappedReconWaste = cols['col-recon-waste'];
                                const mappedReconResource = cols['col-recon-resource'];
                            ...
                            const mappedBlock = cols['col-blockname'];
                            const mappedBench = cols['col-bench'];
                            const mappedSeam = cols['col-seam']; 
                            const mappedSubset = cols['col-subset'];
                            const mappedWaste = cols['col-waste']; 
                            const mappedResource = cols['col-resource']; 
                            const mappedWasteThick = cols['col-waste-thickness'];
                            const mappedResThick = cols['col-resource-thickness'];
                            const mappedQualFrom = cols['col-quality-from'];
                            const mappedQualTo = cols['col-quality-to'];
                            const mappedReconWaste = cols['col-recon-waste']; 
                            const mappedReconResource = cols['col-recon-resource']; 

                            const delimBlock = substrings['delim-block'];
                            const delimStrip = substrings['delim-strip'];
                            const delimBench = substrings['delim-bench'];
                            ...
                            // Menghapus penggunaan identitas baru GROUPNAME pada Header CSV
                            ...
                            for (let i = 1; i < mrLines.length; i++) {
                                ...
                                // Menyusun row tanpa melibatkan GROUPNAME
            ...
            worker.onmessage = async (e) => {
                ...
                if (typeof window.renderGeometryPreview === 'function') {
                    window.renderGeometryPreview(result.combinedCsv, state.summaryObj, result.newBuildMethod);
                }

                try {
                    if (typeof RizpecDB !== 'undefined') await RizpecDB.set(`rizpec_entity_${safePitId}`, result.combinedCsv);
                    await window.savePitMetaToDB(window.activePitId);
                } catch(e) {}

                if (typeof window.updateFolderBadge === 'function') window.updateFolderBadge(window.activePitId, result.newBuildMethod);
                if (typeof window.updateGeometryPitListUI === 'function') window.updateGeometryPitListUI();

                if (action === 'BUILD' && statNewEntity) {
            ...
            // Post the required states to the worker
            worker.postMessage({
                action,
...
// ==============================================================
// GLOBAL PALETTE COLORS APPLIER (Replaces Basic Color from pit.js)
// ==============================================================
window.applyPaletteColors = function() {
    // Dinonaktifkan sesuai kebutuhan: pewarnaan tidak lagi realtime,
    // melainkan menghapus cache geometri dan melakukan render ulang pada Tab Geometry.
};

// Fungsi Baru: Menandai Pit untuk dirender ulang saat parameter warna / mode dirubah
// Catatan: Fungsi ini sekarang dihandle lebih global via tombol "Apply Color"
window.markPitsForRebuildOnColorChange = function(type, changedName) {
    // Logika pengosongan cache digantikan dan dipindahkan ke handler 'btn-apply-colors' 
    // untuk mengakomodir fitur "Apply" manual
};

window.updateApplyColorButton = function() {
...
window.resetUnsavedColorChanges = function() {
    if (window.hasUnsavedColorChanges) {
        window.hasUnsavedColorChanges = false;
        
        // Kembalikan ke versi localStorage secara aman (bukan versi memori yg barusan diedit)
        ...
        if (typeof window.renderPaletteUI === 'function') window.renderPaletteUI();
        
        // Memastikan tombol Apply Color segera disabled ketika kembali ke default
        if (typeof window.updateApplyColorButton === 'function') window.updateApplyColorButton();
    }
};
...
window.initGeometryPitListUI = function() {
    ...
    if (!container) {
        ...
        // Cek secara ketat apakah saat init kita berada murni di Root Folder (Pit Data)
        ...
        container.innerHTML = `
            ...
            <div id="geometry-burden-list" class="flex flex-col gap-1.5 shrink-0"></div>
            ...
            <div id="geometry-subset-list" class="flex flex-col gap-1.5 overflow-y-auto max-h-[150px] custom-scrollbar shrink-0"></div>
            
            <!-- Apply Color Button -->
            <div class="mt-2 shrink-0 pb-3">
                <button id="btn-apply-colors" class="w-full bg-slate-700 text-slate-400 py-1.5 rounded text-[11px] font-bold shadow-lg transition-colors flex items-center justify-center gap-2 cursor-not-allowed" disabled>
        ...
        // Listener tombol Apply Color
        document.getElementById('btn-apply-colors').addEventListener('click', function() {
            if (!window.hasUnsavedColorChanges) return;

            // Simpan ke localStorage SAAT DITEKAN SAJA
            ...
            window.hasUnsavedColorChanges = false;
            window.updateApplyColorButton();

            // Clear semua cache pit agar di-reload secara fresh saat berpindah tab Geometry
            if (window.loadedPits && window.loadedPits.size > 0) {
                ...
                // Beri tanda visual notifikasi ke pengguna
                const tabBtn = document.querySelector('.nav-tab[data-target="panel-geometry"]');
                if (tabBtn) {
    ...
    window.hasUnsavedColorChanges = false; // reset saat UI baru digenerate / list terupdate
    window.updateGeometryPitListUI();
};
...
window.updateGeometryPitListUI = async function() {
        ...
        for (let el of Array.from(pitElements)) {
            ...
            try {
                if (typeof RizpecDB !== 'undefined') {
                    ...
                    if (isGeometryReady) {
                        let hasSubset = (meta.cols && meta.cols['col-subset'] && meta.cols['col-subset'] !== "");
    ...
    // --- Pemrosesan UI Burden Palette (Waste & Resource) ---
    // Di awal, kita ambil state terakhir di LocalStorage
    let burdenPalette = JSON.parse(localStorage.getItem('rizpec_burden_palette')) || 
        ...
        { name: 'Resource', desc: '', color: '#323232' } // Updated Resource color to R50 G50 B50
    ];
    ...
    if (burdenPalette.length !== 2 || burdenPalette[0].name !== 'Waste') {
        ...
        burdenPalette = [
            ...
            { name: 'Resource', desc: resCache?.desc || '', color: resCache?.color || '#323232' } // Updated Resource color
        ];
    }
    
    // Jangan simpan ke localStorage setiap saat (akan disimpan saat Apply ditekan) 
    // Tapi karena kita inisialisasi default, kita save sekali saat pit list terupdate pertama kali jika belum ada.
    ...
    // --- Pemrosesan UI Subset Palette Interaktif ---
    ...
    const getSubsetColor = (name, defIndex) => {
        const upperName = name.toUpperCase();
        if (upperName === 'RESOURCE') return '#323232'; // Updated Resource color to R50 G50 B50
    ...
    // State memori dari rendering Subset saat ini
    window.subsetPalette = currentPalette;
    
    if (typeof window.renderPaletteUI === 'function') {
        window.renderPaletteUI(isEmpty);
    }
    
    window.updateApplyColorButton();
};

window.renderPaletteUI = function(isEmpty = false) {
    const burdenListEl = document.getElementById('geometry-burden-list');
    const subsetListEl = document.getElementById('geometry-subset-list');
        ...
        palette.forEach((item, index) => {
            ...
            div.querySelector('.btn-up').addEventListener('click', () => {
                if (index > 0) {
                    ...
                    // Aktifkan State "Belum Disimpan" lalu Render UI Memory
                    window.hasUnsavedColorChanges = true;
                    if (typeof window.updateApplyColorButton === 'function') window.updateApplyColorButton();
                    window.renderPaletteUI();
                }
            });
...
document.addEventListener('DOMContentLoaded', () => {
    // UI Setup yang spesifik untuk tab Pit Data CSV Configuration
    setTimeout(() => {
        ...
        const mandatoryIds = ['col-blockname', 'col-bench', 'delim-block', 'delim-strip', 'delim-bench', 'col-recon-waste', 'col-recon-resource'];
        ...
        allSettingIds.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('change', async () => { await window.saveCurrentUIState(); window.updateBuildGeometryButtonState(); });
                if (el.tagName === 'INPUT') el.addEventListener('input', async () => { await window.saveCurrentUIState(); window.updateBuildGeometryButtonState(); });
            }
        });
        ...
        navTabs.forEach(tab => {
            tab.addEventListener('click', (e) => {
                ...
                // LOGIKA: Jika user berpindah tab dan belum menekan Apply Color, BATALKAN SEMUA PERUBAHAN
                if (typeof window.resetUnsavedColorChanges === 'function') {
                    window.resetUnsavedColorChanges();
                }
